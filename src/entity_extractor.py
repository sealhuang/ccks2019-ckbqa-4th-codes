...
class SubjectExtractor(object):
    ...
    def get_mention_feature(self, question, mention):
        """Get `mention`'s features."""
        # mention的长度
        f1 = float(len(mention))
        
        try:
            # mention的tf/10000
            f2 = float(self.word_2_frequency[mention])
        except:
            f2 = 1.0
        if mention[-2:] == '大学':
            f2 = 1.0
    ...
    def extract_subject(self, entity_mentions, subject_props, question):
        ...
        for mention in entity_mentions:
            ...
            # 如果它有对应的实体
            if mention in self.mention2entity_dic:
                for entity in self.mention2entity_dic[mention]:
                    # mention的特征
                    mention_features = self.get_mention_feature(
                        question,
                        mention,
                    )
                    # 得到实体两跳内的所有关系
                    ...
                    similar_features = ComputeEntityFeatures(
                        question,
                        entity,
                        relations,
                    )
        ...
        pickle.dump(
            self.entity2hop_dic,
            open('../data/entity2hop_dic.pkl', 'wb'),
        )
    ...
    def GetCandidateEntity(self, corpus):
        ...
        for i in range(len(corpus)):
            ...
            candidate_entity = self.extract_subject(
                entity_mentions,
                subject_props,
                question,
            )
